...
if __name__ == "__main__":
    ...
    agent = Agent()
    # if args.load = True:
    agent.network.q_network.load_state_dict(torch.load('q_network_242_100_good1.pth'))
    ...
    # Train the agent, until the time is up
        ...
        image_size=64
        environment = environment_list[random.randrange(0, 5)]
        state = environment.init_state
    ...
    # Test the agent for 100 steps, using its greedy policy
        #torch.save(agent.network.q_network.state_dict(), 'q_network.pth_delete')
        state = environment.init_state
