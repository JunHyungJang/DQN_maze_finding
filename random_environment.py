...
class Environment:
    ...
    def __init__(self,  magnification, scale):
        ...
        # self.image = np.zeros([1, int(self.magnification * self.height), int(self.magnification * self.width), 3], dtype=np.uint8)
    ...
    def _image(self, agent_state):
        # Create a grey image, representing the environment walls
        self.image.fill(0)
        # Draw all the free blocks, representing the free space
        for block in self.free_blocks:
            top_left = (int(self.magnification * block[0][0]), int(self.magnification * (self.width - block[0][1])))
            bottom_right = (int(self.magnification * block[1][0]), int(self.magnification * (self.width - block[1][1])))
            cv2.rectangle(self.image, top_left, bottom_right, (255, 0, 0), thickness=cv2.FILLED)
        # Draw the agent
        agent_centre = (int(agent_state[0] * self.magnification), int((self.width - agent_state[1]) * self.magnification))
        ...
        # Draw the goal
        goal_centre = (int(self.goal_state[0] * self.magnification), int((self.width - self.goal_state[1]) * self.magnification))
    ...
    def _define_environment_space(self):
        ...
        while prev_right <  0.95*self.width:
            ...
            while not is_within_boundary:
                ...
                top_left = (block_left, block_top)
                bottom_right = (block_right, block_bottom)
                if block_bottom < 0 or block_top > self.height or block_left < 0 or block_right > self.width:
                    is_within_boundary = False
                else:
        ...
        # Set the goal state
        self.goal_state = np.array([self.width-0.05, np.random.uniform(block_bottom + 0.01, block_top - 0.01)], dtype=np.float32)
    ...
    def step(self, state, action):
        ...
        if np.linalg.norm(action) > 0.02:
            ...
            if next_state[0] < 0.0 or next_state[0] > 1.0*self.width or next_state[1] < 0.0 or next_state[1] > 1.0*self.width:
                next_state = state
    ...
    def show(self, agent_state):
        image = self._image(agent_state)
        # Show the image
        cv2.imshow("Environment", image)
        #print(self.image.shape) -> [500,500,3]
